...
#ifndef _SBE_COMMITOFFSETLITE_CXX_H_
#define _SBE_COMMITOFFSETLITE_CXX_H_
...
public:
    ...
    CommitOffsetLite(char *buffer, const std::uint64_t bufferLength) :
        CommitOffsetLite(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    CommitOffsetLite(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        CommitOffsetLite(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    CommitOffsetLite &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    CommitOffsetLite &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    CommitOffsetLite &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        CommitOffsetLite skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    CommitOffsetLite &topicId(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    CommitOffsetLite &sequence(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    CommitOffsetLite &putMessageId(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    CommitOffsetLite &putMessageId(const std::string &str)
    {
        if (str.length() > 65534)
        {
    ...
    #if __cplusplus >= 201703L
    CommitOffsetLite &putMessageId(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    CommitOffsetLite &putMessageIdentifier(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
        std::uint16_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_16(length);
    ...
    CommitOffsetLite &putMessageIdentifier(const std::string &str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    CommitOffsetLite &putMessageIdentifier(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const CommitOffsetLite &_writer)
{
    CommitOffsetLite writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "CommitOffsetLite", )";
